...
BOT_TOKEN = os.getenv('BOT_TOKEN', '8276387203:AAF0ZLplRdELEpaFaqcS0rbKhsW44L5t7do')
...
class Database:
    ...
    def get_user_stats(self, user_id):
        with self.get_connection() as conn:
            ...
            cursor.execute('''
                SELECT SUM(amount) FROM payments WHERE user_id = ? AND status = 'confirmed'
            ''', (user_id,))
...
def get_main_keyboard(user_id=None) -> ReplyKeyboardMarkup:
    ...
    if user_id:
        ...
        if user and (user['free_uses'] > 0 or user['paid_uses'] > 0 or user['is_premium']):
            btn_info = KeyboardButton("🔍 Get Info")
        else:
            btn_info = KeyboardButton("💳 Buy Credits")
    
    btn_help = KeyboardButton("❓ Help")
    btn_recent = KeyboardButton("📜 Recent")
    
    keyboard.add(btn_info, btn_balance)
    keyboard.add(btn_help, btn_recent)
...
def get_payment_keyboard() -> InlineKeyboardMarkup:
    ...
    btn_pay_100 = InlineKeyboardButton("₹100 - 20 Searches + Premium", callback_data="pay_100")
    btn_premium = InlineKeyboardButton("⭐ Premium ₹150/Month", callback_data="pay_premium")
...
def handle_start(message: Message):
    ...
    remaining_text = "Unlimited" if remaining == float('inf') else str(int(remaining))
...
2. Send payment to UPI: <code>{upi}</code>
3. Send screenshot to admin
4. Credits will be added
...
@bot.message_handler(func=lambda message: message.text == "📜 Recent")
def handle_recent(message: Message):
    user_id = message.from_user.id
    
    with db.get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute('''
            SELECT phone_number, search_time FROM search_history
    ...
    recent_message = "📜 <b>Recent Searches</b>\n\n"
    for i, (phone, time) in enumerate(history, 1):
        formatted = format_phone_number(phone)
        recent_message += f"{i}. {formatted}\n   ⏰ {time[:16]}\n\n"
...
📦 <b>Package:</b> {amount == 10 and '2 Searches' or amount == 25 and '5 Searches' or amount == 50 and '10 Searches' or amount == 100 and '20 Searches + 7 Days Premium' or amount == 150 and '30 Days Premium (Unlimited)'}
💰 <b>Amount:</b> ₹{amount}
🆔 <b>Transaction ID:</b> <code>{transaction_id}</code>
...
def handle_add_credits(message: Message):
    ...
    try:
        ...
        bot.send_message(
            message.chat.id,
            f"✅ Added {uses} credits (₹{amount}) to user {target_user}"
        )
    except Exception as e:
        bot.send_message(message.chat.id, f"❌ Error: {e}")
...
def run_bot():
    ...
    logger.info(f"💰 Price per search: ₹{PRICE_PER_SEARCH}")
